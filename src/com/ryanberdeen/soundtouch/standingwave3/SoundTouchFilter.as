...
public class SoundTouchFilter extends FilterSupport implements IAudioFilter {
	...
	public function SoundTouchFilter(source:IAudioSource, pipe:IFifoSamplePipe) {
		super(pipe);
	...
	public function set source(source:IAudioSource):void {
		if (source.descriptor.channels != AudioDescriptor.CHANNELS_STEREO ||
				source.descriptor.rate != AudioDescriptor.RATE_44100) {
			source = new StandardizeFilter(source);
		}
		_source = source;
	}
	...
	public function clone():IAudioSource {
		var result:SoundTouchFilter = new SoundTouchFilter(_source.clone(), pipe.clone());
		return result;
	}
}
}
